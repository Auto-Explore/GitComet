...
; `variable` is a member of three supertypes in this grammar -- `expression`,
; `pattern` and `type` -- so unscoped it matches every lower-case identifier in
; the file, not just a type variable. Overlapping captures resolve last-wins here
; and that rule is 360 lines below the general `(variable) @variable`, so it won
; everywhere: every binding, argument and function call in every Haskell file
; rendered in the type colour. That is the "it is too green" report -- a file with
; one colour on nearly every word.
;
; The fix is to scope it to the supertype the heading already says it belongs to.
; `(name) @type` beside it needs no change: `name` is only ever a type.
;
...
; GitComet: was an unscoped `(variable) @type`; see the header.
(type/variable) @type
