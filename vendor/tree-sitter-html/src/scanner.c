    ...
    while (lexer->lookahead) {
        if (towupper(lexer->lookahead) == end_delimiter[delimiter_index]) {
            ...
            if (delimiter_index == strlen(end_delimiter)) {
                // GitComet: the element closes only when the tag name is followed
                // by whitespace, `/` or `>`.
                //
                // Upstream broke here on the delimiter alone, so the literal
                // characters `</script` ended the element wherever they appeared
                // -- including inside a JavaScript string or comment, which is
                // where they most often appear. The HTML spec's script-data
                // end-tag-name state requires one of those three characters after
                // the name; anything else is text. Without this, a line like
                //
                //     // the characters "</script" end the element
                //
                // closed the block and left the rest of the script as markup.
                advance(lexer);
                if (lexer->lookahead == 0 || iswspace(lexer->lookahead) ||
                    lexer->lookahead == '/' || lexer->lookahead == '>') {
                    break;
                }
                // Not a close after all: keep the text we consumed and carry on.
                delimiter_index = 0;
                lexer->mark_end(lexer);
                continue;
            }
