...
module.exports = grammar({
  ...
  rules: {
    ...
    comment: _ => token(prec(PREC.COMMENT, choice(
      seq('#', /.*/),
      // GitComet: `=end` terminates a block comment only at column 0.
      //
      // Upstream matched the body one *character* at a time, stopping at any
      // `=end` anywhere, and allowed leading whitespace before the terminator.
      // Ruby requires both `=begin` and `=end` to start the line -- which is
      // precisely what a block comment explaining that rule says, and why
      // fixtures/syntax_test/languages/ruby/comments.rb ended its comment on the
      // prose describing it and parsed the next fifty lines as code.
      //
      // Matching whole lines instead is what makes the anchor expressible: every
      // alternative below consumes one complete line that does not begin with
      // `=end`, so the terminator can only be reached at a line start. A line
      // beginning `=endfoo` still terminates, which is Ruby's rule too -- only
