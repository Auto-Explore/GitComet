...
export default grammar({
  ...
  externals: $ => [
    ...
    // GitComet: see `custom_property_declaration` below.
  ...
  rules: {
    ...
    declaration: $ => choice(
      ...
      // GitComet: a custom property takes any balanced token sequence, valid CSS
      // or not, so its value is scanned whole by the external scanner rather than
      // parsed. See `scan_custom_property_value` in src/scanner.c, which declines
      // unless the value actually contains a brace -- everything ordinary CSS can
      // already express keeps its existing tree.
      seq(
    ...
    last_declaration: $ => prec(1, choice(
      seq(
        alias($.identifier, $.property_name),
        ':',
        $._value,
      ...
      // GitComet: the same, for a declaration with no trailing `;`.
      seq(
        alias($.custom_property_name, $.property_name),
        ':',
        choice(
    ...
    _value: $ => prec(-1, choice(
      ...
      // GitComet: `var(--brand)`, `env(--x)`.
      alias($.custom_property_name, $.plain_value),
      $.plain_value,
      $.color_value,
      $.integer_value,
      $.float_value,
    ...
    // GitComet: a custom property's name is its own token.
    //
    // This is what makes the rest work. `--json: {"key": 1};` used to parse as a
    // nested *rule set*: CSS nesting means `name: {` is a perfectly good selector
    // with a pseudo-class colon, so the parser never reached `declaration` at all
    // and the brace opened a block. A selector can never be `--`-prefixed, so
    // giving those names a token of their own removes the ambiguity at the lexer
    // rather than trying to resolve it afterwards.
    //
    // `prec(1)` so it wins over `identifier`, which also matches `--foo`. It is
    // admitted as a value too, because `var(--brand)` is the other place a
    // custom-property name appears.
    custom_property_name: _ => token(prec(1, /--[a-zA-Z0-9-_\xA0-\xFF]*/)),
