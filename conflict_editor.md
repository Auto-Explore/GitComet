...
- ✅ `5. Remove Old Standalone Merge UI` — Rewired `gitgpui-app` mergetool GUI launch to unified `gitgpui_ui_gpui::run_focused_mergetool` + `FocusedMergetoolConfig` (repo path + conflicted file + labels), removed legacy `gitgpui-ui-gpui/src/focused_merge.rs`, and removed legacy exports from `gitgpui-ui-gpui/src/lib.rs`. Ran required `cargo test --workspace --no-default-features --features gix` and `cargo clippy --workspace --no-default-features --features gix -- -D warnings`; both still fail in pre-existing difftool integration and existing clippy issues under `gitgpui-ui-gpui`.
